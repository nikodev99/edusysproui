...
import {AcademicYear, Schedule} from "@/entity";
import { datehelper } from "@/core/helpers/DateHelpers";
...
interface ScheduleReportProps<T extends object> {
...
export const ScheduleReport = <T extends object>(
    {resourceYear, infoData, fetchedSchedules, dataColumn, hasPermission, isSelf, isClasse = false}: ScheduleReportProps<T>
)=> {
    const [weekOffset,   setWeekOffset]   = useState<number>(0);
    const [isAllDay, setIsAllDay] = useState(false)
    const {useGetAllWeekReport} = useTeacherRepo()
    ...
    const {data: reports, refetch} = useGetAllWeekReport(
        infoData?.[dataColumn as keyof T] as string,
